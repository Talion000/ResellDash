...
import { profit, rendement, fmtEur, fmtPct, fmt, daysSince, catBadgeClass, statusClass, groupByMonth, formatMonth } from '../lib/utils'
...
export default function Dashboard() {
  ...
  const catData = useMemo(() => {
    ...
    const catColors = { 'Sneakers': '#3b82f6', 'Pokémon': '#f97316', 'Random': '#22c55e' }
    const cats = Object.entries(map).filter(([, v]) => v > 0)
    return {
      labels: cats.map(([k]) => k),
      data: cats.map(([, v]) => Math.round(v)),
      colors: cats.map(([k]) => catColors[k] || '#888'),
    }
  }, [soldItems])
    ...
    <div style={{ padding: '20px 28px' }}>
      ...
      {alertItems.length > 0 && (
        <div className="alert-banner" style={{ marginBottom: 20 }}>
          ...
          <span><strong style={{ color: 'var(--o)' }}>{alertItems.length} item{alertItems.length > 1 ? 's' : ''}</strong> en stock depuis plus de 90 jours — pense à relancer les prix</span>
        </div>
      )}
      ...
      <div className="kpi-grid" style={{ marginBottom: 20 }}>
        ...
        <div className="kpi-card">
          ...
          <div className="kpi-value" style={{ color: 'var(--b)' }}>{bestCat}</div>
          <div className="kpi-sub" style={{ color: 'var(--b)' }}>par bénéfice total</div>
        </div>
      </div>
      ...
      {monthlyData.labels.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 20 }}>
          <div className="card">
            <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 4 }}>Bénéfice par mois</div>
              ...
              {catData.data.length > 0 ? (
                <>
                  <div style={{ width: 110, height: 110, flexShrink: 0 }}>
                    <Doughnut
                      data={{ labels: catData.labels, datasets: [{ data: catData.data, backgroundColor: catData.colors, borderWidth: 0, borderRadius: 2 }] }}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {catData.labels.map((label, i) => (
                      <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: 'var(--mut)' }}>
                        ...
                        {label}
                        <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--text)', marginLeft: 'auto' }}>+{catData.data[i]}€</span>
                      </div>
                    ))}
                  </div>
                </>
      ...
      <div className="table-container">
        <div className="table-header">
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {[
              { id: 'filterCat', val: filterCat, set: setFilterCat, opts: categories.map(c => c.name), placeholder: 'Toutes catégories' },
              { id: 'filterSt', val: filterSt, set: setFilterSt, opts: ['En stock', 'Vendu', 'Réservé', 'En livraison'], placeholder: 'Tous statuts' },
              { id: 'filterPf', val: filterPf, set: setFilterPf, opts: plateformes, placeholder: 'Toutes plateformes' },
            ].map(f => (
              <select key={f.id} className="form-input" style={{ padding: '5px 10px', fontSize: 11, borderRadius: 20, width: 'auto' }}
            ...
            {filtered.length === 0 ? (
              <tr><td colSpan={9}>
                <div className="empty-state">
                  ...
                  <p>{items.length === 0 ? 'Aucun item pour le moment. Clique sur "+ Ajouter" pour commencer !' : 'Aucun résultat pour ces filtres.'}</p>
                </div>
              </td></tr>
            ) : filtered.map(item => {
              const p = profit(item)
              ...
              return (
                <tr key={item.id} onClick={() => { setEditItem(item); setShowModal(true) }}>
                  ...
                  <td>
                    {p != null ? (
                      <span className={p >= 0 ? 'profit-pos' : 'profit-neg'}>
                        {p >= 0 ? '+' : ''}{fmtEur(p)}
                      </span>
                  ...
                  <td>
                    {r != null ? (
                      <span className={r >= 0 ? 'profit-pos' : 'profit-neg'}>{fmtPct(r)}</span>
                    ) : <span style={{ color: 'var(--mut2)' }}>—</span>}
                  </td>
